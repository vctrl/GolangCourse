...
mutation rateCommentToggle($photoID: ID!, $direction: String!) {
...
function rateCommentToggle(elem) {
    ...
    var params = {
        ...
        operationName: "rateCommentToggle",
    };
...
mutation uploadPhoto($comment: String!, $file: Upload!) { 
    uploadPhoto(comment: $comment, file: $file) { 
        id, comment
    }
}
...
function uploadPhoto(uid) {
    ...
    form.set("map", JSON.stringify({
        my_file: ["variables.file"]
    }));

    // чистим не отсящееся к graphql
    form.delete("csrf-token");
    form.delete("comment");
...
mutation followUser($userID: ID!, $direction: String!) {
