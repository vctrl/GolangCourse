...
func TestServerStreamLeak(t *testing.T) {
	ctx, finish := context.WithCancel(context.Background())
	err := StartMyMicroservice(ctx, listenAddr, ACLData)
	...
	openCloseClient(t, "logger")
	openCloseClient(t, "stats")

	startGoroutineNum := runtime.NumGoroutine()

	wait(1)

	goroutineStat := make([]int, 25)
	for i := 0; i < 25; i++ {
		openCloseClient(t, "logger")
		openCloseClient(t, "stats")
		goroutineStat[i] = runtime.NumGoroutine()
	}

	goroutinesNumAfter25Iterations := runtime.NumGoroutine() - startGoroutineNum
	if goroutinesNumAfter25Iterations > startGoroutineNum {
		t.Fatalf("looks like you have goroutines leak: %+v", goroutineStat)
	}

	finish()
}

func openCloseClient(t *testing.T, consumer string) {
	conn := getGrpcConn(t)

	adm := NewAdminClient(conn)
	consumerCtx2, cancel2 := getConsumerCtxWithCancel(consumer)
	_, err := adm.Statistics(consumerCtx2, &StatInterval{IntervalSeconds: 0})
	if err != nil {
